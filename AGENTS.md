...
| **Administration** | Read & write | Create repos, manage **branch protection** rules and repo settings. ❌ missing (org-repo creation works via org policy, but personal-repo creation and branch protection do not) |
...
- **`main`** — stable, reviewed mainline; the PR target. Treat as protected
  (enable branch protection once the token has `Administration: write`).
...
Then set `main` as the default branch (web UI → Settings → Branches, or
`PATCH /repos/hzmchen/research` with `{"default_branch":"main"}` once the token
has `Administration: write`). If only `dev` exists on an existing repo, create
`main` from the root commit so a PR base exists:
